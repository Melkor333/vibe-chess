...
-define(FILEPATH, "src/modem.gleam").
...
-file("src/modem.gleam", 101).
-spec do_init(fun((gleam@uri:uri()) -> nil)) -> nil.
do_init(_) ->
    nil.

-file("src/modem.gleam", 90).
?DOC(
...
-spec init(fun((gleam@uri:uri()) -> TZE)) -> lustre@effect:effect(TZE).
init(Handler) ->
    lustre@effect:from(
        fun(Dispatch) ->
            gleam@bool:guard(
...
-spec advanced(options(), fun((gleam@uri:uri()) -> TZG)) -> lustre@effect:effect(TZG).
...
-file("src/modem.gleam", 187).
-spec do_replace(gleam@uri:uri()) -> nil.
do_replace(_) ->
    nil.

-file("src/modem.gleam", 209).
-spec do_load(gleam@uri:uri()) -> nil.
do_load(_) ->
    nil.

-file("src/modem.gleam", 201).
?DOC(
...
-file("src/modem.gleam", 233).
-spec do_forward(integer()) -> nil.
do_forward(_) ->
    nil.

-file("src/modem.gleam", 225).
?DOC(
...
-file("src/modem.gleam", 260).
-spec do_back(integer()) -> nil.
do_back(_) ->
    nil.

-file("src/modem.gleam", 252).
?DOC(
    " The browser maintains a history stack of all the url's the user has visited.\n"
...
-spec simulate(
    lustre@dev@simulate:simulation(TZW, TZX),
    lustre@dev@query:'query'(),
    binary(),
    fun((gleam@uri:uri()) -> TZX)
) -> lustre@dev@simulate:simulation(TZW, TZX).
                ...
                fun() ->
                    do_push(
                        {uri,
                            ...
                            Path,
                            gleam@option:then(Query, fun non_empty/1),
                            gleam@option:then(Fragment, fun non_empty/1)}
                    )
                end
            )
        end
